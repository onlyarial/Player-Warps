...
public class PWarpCommand implements CommandExecutor, TabCompleter {
    ...
    private void sendHelp(CommandSender sender, String label) {
        ...
        sender.sendMessage(Text.color("&8Tip: &7Names support spaces, Unicode, & colors, and &#RRGGBB hex colors."));
        ...
        sender.sendMessage(Text.color("&8Tip: &7For spaced rename names, use &e/" + label + " rename Old Name | New Name"));
    ...
    private RenameParts parseRename(String[] args) {
        for (int i = 1; i < args.length; i++) {
            if (args[i].equals("|")) {
                String oldName = WarpNames.join(args, 1, i);
                String newName = WarpNames.join(args, i + 1, args.length);
